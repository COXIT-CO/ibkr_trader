...
# logging.basicConfig(
#     level=logging.INFO,
#     filename=f"taras_trader-{pendulum.now('US/Eastern')}.log",
#     format="%(asctime)s %(message)s",
# )
...
# from taras_trader.helpers import tickFieldsForContract
...
#         order = orders.IOrder(
#             "BUY" if isLong else "SELL", qty, price, outsiderth=True, tif="GTC"  # type: ignore
#         ).order(orderType)
...
#         #     return HTML("No data yet...")  # f"""{self.now:<40}\n""")
#         logger.info("exit")
#         # logger.info(formatTicker("b"))
#         logger.info(self.quoteState)
#         for _ in [formatTicker(quote) for sym, quote in sorted(self.quoteState.items())]:
#             logger.info(_)
#             continue
...
#                     # self.ib.reqMktData(Stock("TSLA", "SMART", "USD"), tickFields)
